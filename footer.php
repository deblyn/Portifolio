			<div id="footer">
	            <div id="esp">
					<p>Especializações:</p>
					<img src="images/php_icon.png" alt="PHP" class="imgfooter" />
					<img src="images/mysql_icon.png" alt="MYSQL" class="imgfooter" />
					<img src="images/css_icon.png" alt="CSS" class="imgfooter" />
					<img src="images/xhtml_icon.png" alt="XHTML" class="imgfooter" />
					<img src="images/dw_icon.png" alt="Dreamweaver CS5" class="imgfooter" />
					<img src="images/flash_icon.png" alt="Flash CS5" class="imgfooter" />
				</div>
                <div id="validator"><a href="http://validator.w3.org/check?uri=referer" title="Validar XHTML W3C"><img src="images/xhtml_w3c.png" alt="XHTML W3C Validator" /></a>
	                    <a href="http://validator.w3.org/check?uri=referer"><img src="images/css_w3c.png" alt="W3C - CSS3 Validator" /></a></div>
					
                
					<div id="assinatura">
                    <p>Todos os Direitos Reservados | Design por DadMoraes<br />Desenvolvido por Deblyn P. Prado - 2011</p></div>
	                
                </div>
                <div id="footerbar">
                	<ul id="redes">
	                	<li><a href="http://twitter.com/deblynprado" id="twitter">twitter</a></li>
	                    <li><a href="" id="facebook">facebook</a></li>
	                    <li><a href="" id="linkedin">Linked<span>in</span></a></li>
	                    <li><a href="" id="aboutme">about<span>.me</span></a></li>
	                    <li><a href="" id="curriculo">currículo</a></li>
                    </ul>
                </div>    
                    
		</div> <!--==| | Fechando a div id="container" | |==-->	
	</body>
</html>